...
public static final String FIND_EMPTY_ROOM="select * from room r left outer join reserv re on r.room_no=re.room_no left outer join room_type rt on rt.room_type_no=r.room_type_no where reserv_check_out<? or reserv_check_in>? or reserv_no is null";
...
public static final String FIND_MY_RESERV_BY_DATE="select * from reserv re join userinfo u on u.user_id = re.user_id join room r on r.room_no= re.room_no where u.user_id =? and reserv_date Between ? and ?+1";
public static final String FIND_ROOM_BY_RESERV_NO = "select r.room_no from room r join reserv re on r.room_no=re.room_no where reserv_no=?";
}
